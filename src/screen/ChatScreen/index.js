...
const getchatroom = (userid, strangerid) => {
	// chtrm = strangerid + 'Chats' + userid
	var chtrm = '1Chats' + userid + 'booking' + 'help'
	console.log("chtrm:_____________", chtrm);
...
const ChatScreen = ({ navigation, route }) => {
	...
	const getfuncchat = () => {
		firestore()
			.collection('AllChat')
			.doc(getchatroom(user_id, admin_id))
			.collection('message')
	...
	const onSend = useCallback(
		(messages = []) => {
			...
			console.log("My Message:_____________", myMessage);
			setMessageslist((previousMessages) => GiftedChat.append(previousMessages, myMessage));
			firestore()
				.collection('AllChat')
				.doc(getchatroom(user_id, admin_id))
